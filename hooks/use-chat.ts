...
export function useChat() {
  ...
  useEffect(() => {
    const savedMessages = localStorage.getItem("digirocket-chat-messages");
    if (savedMessages) {
      try {
  ...
  useEffect(() => {
    if (messages.length > 0) {
      localStorage.setItem("digirocket-chat-messages", JSON.stringify(messages));
    }
  }, [messages]);
  ...
  const sendMessage = async (message: string) => {
    ...
    try {
      await streamCompletion(message);
    } catch (error) {
  ...
  const streamCompletion = async (userMessage: string) => {
    // Initialize OpenAI client
    const openai = new OpenAI({
      baseURL: "https://t1ahx01o6cu6k9zr.us-east-1.aws.endpoints.huggingface.cloud/v1/",
      apiKey: process.env.NEXT_PUBLIC_HUGGINGFACE_API_KEY || "hf_placeholder_key", // Replace with your actual key
      dangerouslyAllowBrowser: true // Only for demo purposes
    });
    
    // The DigiRocket system prompt (abbreviated for brevity)
    const DIGIROCKET_PROMPT = `
...
Avoid generic responses and ensure answers reflect DigiRocket’s global presence, with operations led by a skilled team including Upasana Rathore (Senior Web Developer), Vaishnavi Gupta (Creative Head), and Nitesh Srivastava (SEO Expert). If addressing sensitive topics (e.g., financials), redirect to official channels (e.g., www.digirocket.io for inquiries). For inspiration, mirror the clarity and client focus seen in DigiRocket’s case studies, such as their SEO overhaul for Clean Sips or web revamp for an IT company, and their active social media presence on platforms like Instagram (@digirockett) and LinkedIn.
  ...
  const resetChat = () => {
    setMessages([]);
    setStreamingMessage("");
    localStorage.removeItem("digirocket-chat-messages");
  };
